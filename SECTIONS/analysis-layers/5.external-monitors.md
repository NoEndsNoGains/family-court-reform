...
**Method:** Documentary analysis of TP guidance materials, empirical blog corpus analysis (n=67 posts, November 2024–October 2025), analysis of TIG/FRCTG minutes (n=3 documents, 2021–2024), and cross-layer citation mapping to examine convergence patterns across all five layers.
...
Obtaining-pathway experiences — Barriers encountered by individuals (costs, refusals, delays) leave no footprint. **External observers can describe what they see, but they cannot document what the system does not record.**
...
**2. Documentability Constraint:** The 100%/0% split reveals what the system makes possible to see, not what families find important. The Obtaining pathway is not ignored because it's trivial; it is invisible because it is undocumentable, leaving even expert monitors with no data to analyse.
...
- Confusing Forms (e.g. EX107 with its 7 and 14 days turneround)
...
Internal reform documents (**n=36**, spanning TIG and FRCTG meetings and sub-group sessions) reflect the focus of reform efforts. The distribution below characterises dominant themes, showing how internal reform architects prioritise visible interventions over outcome measurement:
...
**Pattern Recognition:** Like TP's 100%/0% Publication/Obtaining split, TIG/FRCTG's operational priorities are clustered on designing visible reforms. They are not measuring hidden barriers (refusals, costs, penetration rates). This is not a conscious deprioritisation—it's an architectural constraint preventing them from documenting what their own internal systems do not record.
...
This corpus-wide data confirms the TIG’s structural blind spot: the internal reform agenda could not "see" the Obtaining pathway at all.

### 6.5.3.3 The Positional Mobility Test: Dual Institutional Vantage

Having quantified the internal blind spots, we now test whether observational capacity depends on institutional position. The TP chair simultaneously serves on TIG's anonymisation sub-group, creating a natural experiment. If institutional position determined access, TIG membership should reveal data unavailable to external monitors.[^Reed_Dual] It does not.
...
A clean, consistent, and verifiable bibliography is essential. Based on your text, you are using a corpus of 67 TP posts and 36 TIG/FRCTG documents.

Here is your entire bibliography, reformatted for an academic publication (using a clear APA 7-style format), with all 36 TIG/FRCTG links corrected and a uniform access date added.
